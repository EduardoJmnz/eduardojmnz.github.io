/* export.js
   ...
   - Diseñado para usarse como ES Module (Opción 1).
   ...
   const exporter = createExporter({
     ...
     // helpers que ya tienes en app.js:
     clamp,
     cmToPx,                 // (cm, dpi) => px
     computeRenderTokens,
     getStyleDef,
   ...
   // y en tu botón Exportar:
   // exporter.exportPoster(state.export.format, state.export.sizeKey);
*/
...
export function createExporter(core) {
  ...
  // Dibuja texto usando la misma caja/align/font del DOM (preview)
  function drawDomTextLikePreview(ctx, tokens, el, text, sx, sy, familyFallback) {
    ...
    // X según alineación y caja real
    const xPx =
      (canvasAlign === "left")
        ? rr.x
        : (canvasAlign === "center")
    ...
    // Font size (auto-ajustada en preview) -> escalada
    const fontSizeBase = parseFloat(cs.fontSize || "16");
    let size = fontSizeBase * sy;

    // Mini-fit por seguridad para que no se salga de la caja del DOM
    const maxW = rr.w * sx;
  ...
  async function exportPoster(format, sizeKey) {
    ...
    // Reglas marco/margen (igual a tu lógica)
    const frameOn = (!isPoster()) && !!state.map.posterFrameEnabled;
    const marginOn = (!isPoster()) && !!state.map.posterMarginEnabled && !frameOn;

    const POSTER_FRAME_EDGE_GAP_PX = 0;   // en tu código actual es 0
    const POSTER_MARGIN_EDGE_GAP_PX = 50; // en tu código actual es 50
    ...
    // Mapa (usa el canvas actual del preview)
    ...
    const mapY = Math.round(innerY + (70 * sy)); // igual a tu layout (innerTop=70)
    ...
    // ✅ Texto calcado del preview
    // ---------------------------
    // Importante: asegurar que el preview ya está en su estado final
    // antes de medir (auto-fit y DOM).
    try {
      renderPosterText();
      applyAutoTextSizing();
    } catch (_) {
      // si el core aún no expone estas funciones, no rompemos export.
    }
