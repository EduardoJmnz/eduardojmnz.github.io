...
export function createExporter(core) {
  ...
  // ---------------------------
  function getRectRelToPoster(el, posterRoot = posterEl) {
    if (!el) return null;
    const pr = posterRoot.getBoundingClientRect();
  ...
  function drawDomTextLikePreview(ctx, tokens, el, text, sx, sy, familyFallback, posterRoot) {
    if (!el) return;
    if (!text) return;

    const rr = getRectRelToPoster(el, posterRoot);
    if (!rr) return;
  ...
  async function exportPoster(format, sizeKey) {
    ...
    const sy = H / POSTER_H;
// ===========================
// ✅ MEDICIÓN "PIXEL PERFECT"
// Clonamos el póster fuera del preview (sin scale/zoom responsive)
// para que las posiciones de texto coincidan EXACTO con el layout real.
// ===========================
function createExportMeasureClone() {
  const clone = posterEl.cloneNode(true);
  clone.id = "posterExportClone";
...
const cleanupClone = createExportMeasureClone();
const posterRoot = document.getElementById("posterExportClone") || posterEl;

const $titleEl  = posterRoot.querySelector("#pTitle");
const $subEl    = posterRoot.querySelector("#pSubtitle");
const $placeEl  = posterRoot.querySelector("#pPlace");
const $coordsEl = posterRoot.querySelector("#pCoords");
const $dtEl     = posterRoot.querySelector("#pDatetime");
    ...
    if (state.visible?.title)    drawDomTextLikePreview(ctx, tokens, $titleEl,  title, sx, sy, family, posterRoot);
    if (state.visible?.subtitle) drawDomTextLikePreview(ctx, tokens, $subEl,    sub,   sx, sy, family, posterRoot);
    if (state.visible?.place)    drawDomTextLikePreview(ctx, tokens, $placeEl,  state.text?.place || "",  sx, sy, family, posterRoot);
    if (state.visible?.coords)   drawDomTextLikePreview(ctx, tokens, $coordsEl, state.text?.coords || "", sx, sy, family, posterRoot);
    if (state.visible?.datetime) drawDomTextLikePreview(ctx, tokens, $dtEl,     getDateTimeString(),      sx, sy, family, posterRoot);

    // ✅ limpiar clon de medición
